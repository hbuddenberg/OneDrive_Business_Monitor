...
class OneDriveChecker:
                ...
                if any(k in ps_lower for k in sync_keywords):
                     ...
                     if age > SYNC_TIMEOUT:
                         ...
                         if "pendiente" in ps_lower or "pending" in ps_lower:
                             ...
                             # Check if tray tooltip shows auth required even though file says pending
                             if self.check_tray_auth_required():
                                 logger.warning("Verificación Activa: Tray muestra Auth Required mientras archivo está 'Pendiente'")
                                 return OneDriveStatus.AUTH_REQUIRED, "Autenticación Requerida (Detectado en Tooltip)"
    ...
    def check_tray_auth_required(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Tray Auth Check - Method 1: Checking {sync_diag_log}")
            if sync_diag_log.exists():
                try:
                    ...
                    if found_indicators:
                        logger.warning(f"Tray Auth: Found indicators in log: {found_indicators}")
                        return True
                    else:
                        logger.info("Tray Auth Check - Method 1: No auth indicators found in log")
                            
                except Exception as e:
                    logger.debug(f"Error reading SyncDiagnostics.log: {e}")
            else:
                logger.info("Tray Auth Check - Method 1: Log file not found")
            ...
            logger.info(f"Tray Auth Check - Method 2: Checking window titles (target: {target_title})")
            ps_script = f"""
$procs = Get-Process -Name OneDrive -ErrorAction SilentlyContinue
foreach ($proc in $procs) {{
    $title = $proc.MainWindowTitle
    Write-Output "ONEDRIVE_PROC:$($proc.Id)|$title"
...
"""
            ...
            if result.returncode == 0:
                ...
                if output:
                    logger.info(f"Tray Auth Check - Method 2: PowerShell output:\n{output}")
                else:
                    logger.info("Tray Auth Check - Method 2: No window titles found")
                    
                if "AUTH_WINDOW:" in output or "LOGIN_WINDOW:" in output or "ONEDRIVE_AUTH:" in output:
                    logger.warning(f"Tray Auth Required Detected via window title")
                    return True
            else:
                logger.debug(f"Tray Auth Check - Method 2: PowerShell error: {result.stderr}")
            
            # Method 3: Check Shell column 202 ("No resuelto" = unresolved sync issue)
            # and check column 305 for specific auth keywords
            logger.info("Tray Auth Check - Method 3: Checking Shell columns 202 and 305")
            try:
                ps_script = f"""
$shell = New-Object -ComObject Shell.Application
$folder = $shell.Namespace('{self.canary_path.parent}')
$item = $folder.ParseName('{self.canary_path.name}')
if ($item) {{
    $col202 = $folder.GetDetailsOf($item, 202)
    $col305 = $folder.GetDetailsOf($item, 305)
    Write-Output "COL202:$col202"
    Write-Output "COL305:$col305"
}}
"""
                result = subprocess.run(
                    ["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", ps_script],
                ...
                if result.returncode == 0 and result.stdout.strip():
                    output = result.stdout.strip()
                    logger.info(f"Tray Auth Check - Method 3: Shell output:\n{output}")
                    
                    # Check for auth-related text in the column values
                    output_lower = output.lower()
                    auth_keywords = ["credencial", "sign in", "iniciar ses", "autenticacion", "vuelve"]
                    for kw in auth_keywords:
                        if kw in output_lower:
                            logger.warning(f"Tray Auth: Found '{kw}' in Shell status columns")
                            return True
                else:
                    logger.info("Tray Auth Check - Method 3: No Shell output")
                    
            except Exception as e:
                logger.debug(f"Error checking Shell columns: {e}")
            
            # Method 4: Check Windows notification database (most reliable)
            logger.info("Tray Auth Check - Method 4: Checking Windows notification database")
            try:
                import sqlite3
                db_path = Path(os.environ['LOCALAPPDATA']) / 'Microsoft' / 'Windows' / 'Notifications' / 'wpndatabase.db'
                
                if db_path.exists():
                    conn = sqlite3.connect(str(db_path))
                    cursor = conn.cursor()
                    
                    # Find OneDrive handler IDs
                    cursor.execute("""
                        SELECT RecordId FROM NotificationHandler 
                        WHERE PrimaryId LIKE '%OneDrive%' OR PrimaryId LIKE '%SkyDrive%'
                    """)
                    handler_ids = [row[0] for row in cursor.fetchall()]
                    
                    if handler_ids:
                        # Get recent notifications (last 5 minutes only)
                        # ArrivalTime is in Windows FILETIME format (100-nanosecond intervals since 1601-01-01)
                        import time
                        # Calculate min_arrival_time: current time - 5 minutes in FILETIME format
                        # FILETIME epoch: Jan 1, 1601 | Unix epoch: Jan 1, 1970
                        # Difference: 11644473600 seconds
                        EPOCH_DIFF = 11644473600
                        TICKS_PER_SECOND = 10000000
                        current_filetime = (int(time.time()) + EPOCH_DIFF) * TICKS_PER_SECOND
                        min_arrival_time = current_filetime - (5 * 60 * TICKS_PER_SECOND)  # 5 minutes ago
                        
                        ids_str = ','.join(str(h) for h in handler_ids)
                        cursor.execute(f"""
                            SELECT Payload, ArrivalTime FROM Notification 
                            WHERE HandlerId IN ({ids_str})
                            AND ArrivalTime >= {min_arrival_time}
                            ORDER BY ArrivalTime DESC
                            LIMIT 10
                        """)
                        
                        auth_keywords = ['vuelve', 'credential', 'credencial', 'sign in', 'iniciar ses', 'escribir']
                        found_recent = False
                        
                        for row in cursor.fetchall():
                            payload = row[0]
                            arrival_time = row[1] if len(row) > 1 else 0
                            if payload:
                                try:
                                    text = payload.decode('utf-8', errors='ignore').lower()
                                    for kw in auth_keywords:
                                        if kw in text:
                                            # Calculate notification age for logging
                                            if arrival_time:
                                                age_seconds = (current_filetime - arrival_time) / TICKS_PER_SECOND
                                                logger.warning(f"Tray Auth: Found '{kw}' in Windows notification (age: {age_seconds:.0f}s)")
                                            else:
                                                logger.warning(f"Tray Auth: Found '{kw}' in Windows notification")
                                            conn.close()
                                            return True
                                except Exception:
                                    pass
                        
                        logger.info("Tray Auth Check - Method 4: No auth keywords in recent notifications (last 5 min)")
                    else:
                        logger.info("Tray Auth Check - Method 4: No OneDrive handlers found")
                    
                    conn.close()
                else:
                    logger.info("Tray Auth Check - Method 4: Notification database not found")
                    
            except Exception as e:
                logger.debug(f"Error reading notification database: {e}")
                    
            logger.info("Tray Auth Check: No auth required detected")
            return False
