        ...
        if status != self.last_status or self.is_first_run:
            ...
            if status == OneDriveStatus.SYNCING and syncing_timeout > 0 and actual_time_in_state >= syncing_timeout:
                if not self._in_cooldown():
                    # Send SYNC_TIMEOUT notification before restart
                    duration_mins = actual_time_in_state / 60
                    duration_str = f"{duration_mins:.1f} minutos ({actual_time_in_state:.0f} segundos)"
                    logger.warning(f"SYNC_TIMEOUT: SYNCING state persisted for {actual_time_in_state:.0f}s (timeout: {syncing_timeout}s). Sending notification and forcing restart.")
                    
                    try:
                        self.notifier.send_status_notification(
                            status="SYNC_TIMEOUT",
                            timestamp=timestamp_str,
                            message=f"La sincronización ha estado activa por {duration_str} sin completarse. Se intentará reiniciar OneDrive.",
                            duration=duration_str
                        )
                        logger.info(f"SYNC_TIMEOUT: Sent sync timeout notification.")
                    except Exception as e:
                        logger.error(f"Failed to send SYNC_TIMEOUT notification: {e}")
